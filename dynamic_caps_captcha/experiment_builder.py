...
class ExperimentBuilder(nn.Module):
    def __init__(self, network_model, experiment_name, num_epochs, train_data, val_data,
        ...
        # self.recon_loss_scale = 10
        # self.criterion = CapsuleLoss().to(self.device)
    ...
    def run_train_iter(self, x, y):
        ...
        :param y: The targets for the model. A tensor of shape (batch_size, 4)
        ...
        # plt.show()
        # plt.imshow(x_mask[0].permute(1, 2, 0).cpu())
        # plt.show()
        # print(y[0].cpu())

        # outputs (b, num_caps_output = 4, num_classes = 10) reconstructions (b, c, h, w)
        # outputs, reconstructions = self.model(x)
        ...
        # print(predicted[0])
        loss = self.class_loss(outputs.view(-1 , 10), y.view(-1))
               # self.recon_loss_scale * self.recon_loss(reconstructions, x_mask)


        # print(self.recon_loss(reconstructions, x_mask))
        self.optimizer.zero_grad()
        loss.backward()
        self.optimizer.step()
        # plt.imshow(reconstructions[0].detach().permute(1, 2, 0).cpu())
        # plt.show()
        accuracy = np.mean(list((predicted.eq(y.data)).view(-1).cpu()))
        real_acc = np.mean(list(torch.all((predicted.eq(y.data)), dim=1).cpu()))

        return loss.data.detach().cpu().numpy(), accuracy, real_acc
    ...
    def run_training_epoch(self, current_epoch_losses):
        with tqdm.tqdm(total=len(self.train_data), file=sys.stdout) as pbar_train:  # create a progress bar for training
            for idx, (x, y) in enumerate(self.train_data):  # get data batches
                loss, accuracy,real_acc = self.run_train_iter(x=x, y=y)  # take a training iter step
    ...
    def run_validation_epoch(self, current_epoch_losses):
        ...
        with tqdm.tqdm(total=len(self.val_data), file=sys.stdout) as pbar_val:  # create a progress bar for validation
            for x, y in self.val_data:  # get data batches
                loss, accuracy, real_acc = self.run_evaluation_iter(x=x, y=y)  # run a validation iter
    ...
    def run_testing_epoch(self, current_epoch_losses):
        ...
        with tqdm.tqdm(total=len(self.test_data), file=sys.stdout) as pbar_test:  # ini a progress bar
            for x, y in self.test_data:  # sample batch
                loss, accuracy, real_acc = self.run_evaluation_iter(x=x,
    ...
    def run_experiment(self):
        ...
        current_epoch_losses = {"test_acc": [], "test_real_acc": [], "test_loss": []}  # initialize a statistics dict
