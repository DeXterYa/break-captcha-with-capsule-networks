...
class PrimaryCapsules(nn.Module):
    ...
    def build_module(self):
        ...
        with torch.no_grad():
            ...
            x_1 = torch.zeros(self.input_shape)
            x_2 = torch.zeros(self.input_shape)
            # Compute the number of capsules in the Primary Capsules layer
            self.num_caps = int(self.out_channels / self.dim_caps)
            self.layer_dict['conv_1'] = nn.Conv2d(in_channels=self.input_shape[1],
                                                out_channels=self.out_channels,
                                                kernel_size=self.kernel_size,
                                                stride=self.stride,
                                                padding=self.padding)
            self.layer_dict['conv_2'] = nn.Conv2d(in_channels=self.input_shape[1],
                                                  out_channels=self.out_channels,
                                                  kernel_size=self.kernel_size,
                                                  stride=self.stride,
                                                  padding=self.padding)

            out_1 = self.layer_dict['conv_1'](x_1)
            out_2 = self.layer_dict['conv_2'](x_2)

            print("The shape of feature maps after convolution: ", out_1.shape)
            # Group dim_caps pixel values together across channels as elements of a capsule vector
            # h = out.size(2)
            # w = out.size(3)
            out_1 = out_1.view(out_1.size(0), self.num_caps, self.dim_caps, out_1.size(2), out_1.size(3))
            out_2 = out_2.view(out_2.size(0), self.num_caps, self.dim_caps, out_2.size(2), out_2.size(3))
            ...
            out_1 = out_1.permute(0, 1, 3, 4, 2).contiguous()
            out_2 = out_2.permute(0, 1, 3, 4, 2).contiguous()
            ...
            out_1 = out_1.view(out_1.size(0), -1, out_1.size(-1))
            out_2 = out_2.view(out_2.size(0), -1, out_2.size(-1))
...
class DynamicCapsules(nn.Module):
    def __init__(self, input_shape, num_channels_primary, dim_caps_primary,
                 ...
                 kernel_size=9, dropout=False, coord=False, attention=False, stride=1, random_order=True,
    ...
    def build_module(self):
        with torch.no_grad():
            ...
            self.layer_dict['conv2'] = nn.Conv2d(self.input_shape[1], self.num_channels_primary,
                                                 self.kernel_size, stride=self.stride, bias=True)

            out_1 = self.layer_dict['conv1'](x_1)
            out_2 = self.layer_dict['conv1'](x_2)
            print("shape after conv1: ", out_1.shape)
            ...
            print("primary cap channels: ", self.num_channels_primary)
            self.layer_dict['primarycaps'] = PrimaryCapsules(out_1.shape, self.num_channels_primary,
                                                             self.dim_caps_primary, self.kernel_size,
                                                             coord=self.coord, attention=self.attention)
            out = self.layer_dict['primarycaps'](out_1, out_2)
            # Only output 4 capsules because there are only 4 digits in the image
            self.layer_dict['digitcaps'] = RoutingCapsules(out.shape, 3, self.dim_caps_output,
